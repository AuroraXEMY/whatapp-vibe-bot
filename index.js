...
function getGroupConfig(jid) {
  if (!groupConfigCache.has(jid)) {
    ...
    groupConfigCache.set(jid, {
      ...
      movieModeEnabled: true, newsletterEnabled: true, lastNewspaperDate: null,
...
async function loadGroupConfigsFromMongo() {
  ...
  try {
    ...
    for (const doc of all) {
      ...
      // NaN min". These three fields are deliberately session-only (never
      // persisted to Mongo — see the comment in getGroupConfig), so on load
      // they must be freshly initialized here, not read from `doc`.
...
async function maybeGenerateDailyNewspaper(sock) {
  if (!sock || PROVIDER_CHAIN.length === 0) return;
  // Only the scheduler ticks that land inside the target hour do any real
  // work below — cfg.lastNewspaperDate then stops it firing more than once
  // per group even though several 10-min ticks fall inside that hour.
  if (new Date().getHours() !== NEWSPAPER_HOUR) return;
  const today = todayKey();
...
async function handleCommand(sock, jid, senderJid, sender, text, msg) {
  ...
  try {
    ...
    if (cmd === ".help" || cmd === ".menu") {
      await sock.sendMessage(jid, {
        text: `🤖 *${BOT_CONFIG.name} Commands*\n\n*Everyone:*\n.rank / .level — your XP & title\n.stats — bot health\n.health (or .info / .bot) — full system diagnostics\n.mood — show current personality\n.about — what I am\n.owner — who made me\n.search <query> — I'll look it up on the web\n.imagine <prompt> — I'll generate an image (or just ask me naturally, like "draw me a cat")\n.sticker (reply to an image/sticker) — turn it into a proper sticker\n.tts <question> (optionally replying to an image/vn/link/anything) — I'll explain it as a voice note\n.truth / .dare — a fresh one every time, or just say "let's play truth or dare"\n.quote — a deep/inspiring quote, or just ask me for one\n.eli5 <topic> — I'll explain it like you're 5\n.ping / .flip / .roll [sides] / .8ball — quick fun stuff\n\n*Group admins only:*\n.settings — this group's status (mute, mood, message counts)\n.activity — 7-day activity heatmap\n.lock / .unlock — restrict messaging to admins\n.mood <name> — change personality (${AVAILABLE_MOODS.join(", ")})\n.moviemode on/off — toggle the daily recap\n.newsletter on/off — toggle the Daily Newspaper\n.kick (reply or @mention) — remove a member\n.promote / .demote (reply or @mention) — admin toggle\n.tagall — mention everyone\n.del (reply to a message) — delete it\n.mute / .unmute — I go completely silent in this group until unmuted\n.ignore (reply or @mention) — I stop responding to that one person here\n.ignorelist / .undoignore (@user or "all") — manage the ignore list\n\nTag me or say my name to chat — send me a photo/sticker/voice note directly and I'll understand it too, and I can analyze an image/link/write-up all together if you reply to it and ask me to explain! Heads up: I'm a vibe bot, not a moderator — I don't delete stuff or hand out formal warnings, ever.`
      }, { quoted: msg });
      return true;
    }
    ...
    if (jid.endsWith("@g.us") && (cmd === ".newsletter" || cmd.startsWith(".newsletter "))) {
      ...
      const arg = text.trim().split(/\s+/)[1]?.toLowerCase();
      if (arg !== "on" && arg !== "off") {
        await sock.sendMessage(jid, { text: `📰 Daily Newspaper is currently *${getGroupConfig(jid).newsletterEnabled ? "ON" : "OFF"}*.\nUsage: *.newsletter on* or *.newsletter off*` }, { quoted: msg });
        return true;
      }
      const cfg = getGroupConfig(jid);
