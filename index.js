...
function checkDuplicateSpam(senderJid, text) {
  ...
  if (entry?.cooldownUntil > now) return "blocked"; // already in the 5-min penalty box
...
function getUserStats(jid) {
  ...
  const stats = userStatsCache.get(jid);
  // Lightweight memory expiry: if it's been over a month since this person
  // was last active, the little "facts" the bot remembered about them fade
  // out naturally (XP/level are untouched — only the personal-facts memory
  // resets). Checked lazily here rather than a scheduled job — cheap, no
  // extra infrastructure, satisfies "memory that resets after a month"
  // using what already exists instead of building a new profile system.
  if (stats.facts.length > 0 && stats.lastActive && (Date.now() - new Date(stats.lastActive).getTime() > FACT_MEMORY_TTL_MS)) {
    stats.facts = [];
    stats.dirty = true;
  }
  return stats;
...
async function searchWeb(query) {
  ...
  for (const key of TAVILY_KEYS) {
    try {
      ...
      console.warn(`⚠️ [SEARCH] Tavily key failed (${err.message}) — trying next key if any...`);
      continue;
    }
  }
  return { success: false, results: null };
...
async function analyzeImageWithGemini(base64Image, mimeType, question) {
  if (GEMINI_VISION_KEYS.length === 0) {
  ...
  for (const key of GEMINI_VISION_KEYS) {
    try {
      ...
      if (!response.ok) throw new Error(`Gemini vision returned HTTP ${response.status}`);
      const data = await response.json();
      return { success: true, message: data.choices[0].message.content.trim() };
    } catch (err) {
      console.warn(`⚠️ [VISION] Gemini key failed (${err.message}) — trying next key if any...`);
      continue;
    }
  }
...
async function transcribeAudioWithGroq(audioBuffer, mimeType) {
  ...
  for (const key of groqKeys) {
    try {
      ...
      if (!response.ok) throw new Error(`Groq Whisper returned HTTP ${response.status}`);
      const data = await response.json();
      return { success: true, text: (data.text || "").trim() };
    } catch (err) {
      console.warn(`⚠️ [TRANSCRIBE] Groq key failed (${err.message}) — trying next key if any...`);
      continue;
    }
  }
  return { success: false, text: null };
...
function describeAIError(err) {
  ...
  if (status === 400 || msg.includes("context length") || msg.includes("context_length") || msg.includes("too many tokens") || msg.includes("bad request")) {
    return { category: "PAYLOAD_TOO_LARGE", detail: "Prompt exceeded a provider's token/context limit or was otherwise malformed.", userText: "😵‍💫 Okay, that was way too much text for my brain to process at once — try a shorter message!" };
  }
...
async function generateAIChatReply(senderJid, sender, question, vibe = BOT_CONFIG.vibe, context = "", quotedText = null, feelingSalty = false, searchContext = "") {
  if (PROVIDER_CHAIN.length === 0) {
...
Respond ONLY with a raw JSON object matching this schema, no other text:
{
  ...
  "newFact": "one short new casual/non-sensitive fact worth remembering about this person from this message, or an empty string if nothing notable"
    ...
    ], { json: true, temperature: 0.8, timeoutMs: 20000, maxTokens: 700 });
        ...
        if ((incomingMediaType === "image" || incomingMediaType === "sticker") && mediaEligible) {
          ...
          try {
            ...
            if (imageBuffer) {
              ...
              const mimeType = incomingMediaType === "sticker" ? "image/webp" : "image/jpeg";
        ...
        if (wantsSummary) {
          ...
          if (shouldAutoSearch(text, vibe)) {
            ...
            try {
              ...
              if (searchResult.success && searchResult.results) searchContext = searchResult.results;
            } catch (err) {
          ...
          aiResult = await generateAIChatReply(senderJid, sender, boundedQuestion, vibe, context, quotedText, feelingSalty, searchContext);
        }
